...
class StationaryCenterFinder:
    ...
    def __call__(self, c0, w0, x, y, tec):
        ...
        result = result_list[np.argmax([r["metric"] for r in result_list])]
...
def initial_center_from_spectral_vectors(
    px: np.ndarray,
    py: np.ndarray,
    Fx: np.ndarray,
    Fy: np.ndarray,
    ...
    """Estimate radial center and initial wavelength from patch vectors."""

    X, Y = np.meshgrid(px, py)
    pts = np.column_stack((X.ravel(), Y.ravel()))
    vectors = np.column_stack((Fx.ravel(), Fy.ravel()))
    k = np.hypot(vectors[:, 0], vectors[:, 1])
    center = find_center(pts, vectors, weights.ravel())
    wavelength = 1 / np.nanmax(k[k > 0])
    return center, wavelength


def fit_radial_center(
    c0,
    w0,
    x,
    y,
    ...
    *,
    ...
    improvement: float = 1.0e-5,
    steps: int = 50,
):
    """Fit a radial center with the legacy correlation model."""

    finder = CenterFinder(max_iter, learning_rate, improvement, steps)
    return finder(c0, w0, x, y, tec)


def fit_stationary_center(
    c0,
    w0,
    x,
    y,
    ...
    *,
    ...
    """Fit a fixed center and time-varying wavelength with the legacy model."""

    finder = StationaryCenterFinder(max_iter, learning_rate, num_starts, reg_strength)
    return finder(c0, w0, x, y, tec)
