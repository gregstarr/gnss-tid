...
def optimize(r, tid, cfg, height):
    best_metric = -np.inf
    for _start in tqdm.tqdm(range(cfg.opt.n_inits), f"{height=}"):
...
def run_time_index(points, time_slice, cfg):
    logging.info(
        "running times: %s - %s",
        points.time[time_slice.start],
        points.time[time_slice.stop],
    )
...
def plot_results(points: PointData, results: list, time_slice: slice):
    ...
    plot_radial2(
        ax[0], (best.cx, best.cy), best.wavelength, best.wavelength2, best.offset
    )
    ax[0].set_title(f"center=({best.cx}, {best.cy})")

    ax[1].axvline(best.height, color="k", linestyle="--", alpha=0.5)
