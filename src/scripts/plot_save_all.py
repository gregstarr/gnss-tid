...
def ImageNew(
    ...
    """ """
    x_edges = np.arange(lonlim[0], lonlim[1] + 0.01, res)
    y_edges = np.arange(latlim[0], latlim[1] + 0.01, res)
    xgrid, ygrid = np.meshgrid(x_edges, y_edges)
    ny, nx = xgrid.shape

    imout = np.full((ny, nx), np.nan)
    varout = np.full((ny, nx), np.nan)
    cnt = np.zeros((ny, nx), dtype=int)

    idx = np.digitize(glon, xgrid[0, :]) - 1
    idy = np.digitize(glat, ygrid[:, 0]) - 1

    valid = (idx >= 0) & (idx < nx) & (idy >= 0) & (idy < ny)

    # Additionally enforce the "within 3*res" proximity check
    col_centres = x_edges[idx[valid]]
    row_centres = y_edges[idy[valid]]
    close = (np.abs(glon[valid] - col_centres) < 3 * res) & (
        np.abs(glat[valid] - row_centres) < 3 * res
    )
    valid_idx = np.where(valid)[0][close]

    glon_v = glon[valid_idx]
    glat_v = glat[valid_idx]  # noqa: F841  (kept for clarity)
    tid_v = tid[valid_idx]
    var_v = variance[valid_idx]
    col_v = idx[valid_idx]
    row_v = idy[valid_idx]

    # Encode each (row, col) pair as a single integer key
    cell_key = row_v * nx + col_v

    # Sort everything by cell key so identical cells are contiguous
    ...
    split_at = np.flatnonzero(np.diff(cell_key)) + 1
    tid_groups = np.split(tid_s, split_at)
    var_groups = np.split(var_s, split_at)

    # Unique cells (one per group)
    unique_keys = cell_key[np.concatenate(([0], split_at))]
    unique_rows = unique_keys // nx
    unique_cols = unique_keys % nx

    for k, (tg, vg) in enumerate(zip(tid_groups, var_groups)):
        r, c = unique_rows[k], unique_cols[k]
        imout[r, c] = np.nanmedian(tg)
        varout[r, c] = np.nanmax(vg)  # conservative: worst variance wins
        cnt[r, c] = tg.size
    ...
    if filter_type == "median":
        ...
        kernel = Gaussian2DKernel(
            x_stddev=sigma, y_stddev=sigma, x_size=filter_size, y_size=filter_size
        )
        imout = convolve(imout, kernel)
        varout = convolve(varout, kernel)
        for arr in (imout, varout):
            arr[:filter_size, :] = np.nan
            arr[:, :filter_size] = np.nan
            arr[-filter_size:, :] = np.nan
            arr[:, -filter_size:] = np.nan
    else:
...
def main(P):
    ...
    if P.tlim is None:
        tlim = [
            datetime.strptime(datefmt, "%Y/%m%d").strftime("%Y%m%d_0000"),
            (datetime.strptime(datefmt, "%Y/%m%d") + timedelta(days=1)).strftime(
                "%Y%m%d_0000"
            ),
        ]
    else:
  ...
  points = gnss_tid.pointdata.PointData(
    ...
    for i in range(len(time_slices)):
        ...
        ofn = (
            ncfn
            + "grid_"
            + times[i].astype("datetime64[s]").astype(datetime).strftime("%Y%m%dT%H%M")
            + ".nc4"
        )
        ...
        O = xr.Dataset(
            coords={  # "lon": (["lat","lon"], xgrid), "lat": (["lat","lon"], ygrid),
                "ipp_lon": point_data.lon.values,
                "ipp_lat": point_data.lat.values,
            },
            data_vars={
                "dtec30": (["ipp"], point_data.dtec1.values),
                "dtec60": (["ipp"], point_data.dtec2.values),
                "dtec90": (["ipp"], point_data.dtec3.values),
                "roti": (["ipp"], point_data.roti.values),
        ...
        encoding = dict.fromkeys(O.data_vars, comp)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "-latlim",
        help="latlim, 2 aruments. Default = -90, 90",
        nargs=2,
        type=int,
        default=[-90, 90],
    )
    p.add_argument(
        "-lonlim",
        help="lonlim, 2 aruments. Default = -180, 180",
        nargs=2,
        type=int,
        default=[-180, 180],
    )
    ...
    p.add_argument(
        "-res", help="Grid resolution. Float. Default=0.2", default=0.2, type=float
    )
    p.add_argument("-filter", help="Gaussian or median.", default="gaussian")
    p.add_argument(
        "-skip",
        help="Time Resolution? Default=30. 30*30sec = 15min",
        default=30,
        type=int,
    )
    p.add_argument(
        "-avg", help="Average in Time? Default=10. 10*30sec = 5min", default=10, type=int
    )
    p.add_argument(
        "-tlim",
        help="start and end times, 2 args. YYYYmmdd_HHMM YYYYmmdd_HHMM",
        default=None,
        nargs=2,
        type=str,
    )
    p.add_argument(
        "-filtersize",
        help="filter size. Needs three arguments. Default = 9,11,13",
        default=[5, 9, 9],
        nargs=3,
        type=int,
    )
    p.add_argument(
        "-filtersigma",
        help="filter sigma. sigma_y=sigma_x. Needs three arguments. Default = 0.75, 1.5, 3",
        default=[0.75, 1, 1.5],
        nargs=3,
        type=float,
    )
    p.add_argument(
        "-j",
        help='Number of processes for reading in the data" Default=4',
        default=4,
        type=int,
    )
